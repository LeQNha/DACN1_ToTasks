...
import pandas as pd
# from utils.ToolsPreparation import le_type, le_importance, le_day, ohe_type
...
def predict_start_time_2(task_name, task_type, importance, day_of_week):
    ...
    task_type_encoded = pd.DataFrame([[task_type]], columns=["Type"])
    task_type_onehot = ohe_type.transform(task_type_encoded).toarray().flatten()
    ...
    X_new = np.concatenate([
        task_name_vectorized.toarray().flatten(),
        task_type_onehot,
        [importance_encoded, day_of_week_sin, day_of_week_cos]
    ]).reshape(1, -1)
